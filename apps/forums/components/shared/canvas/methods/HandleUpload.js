...
export default function HandleUpload(props) {
    try {
        ...
        reader.onload = newData => {
            console.log(newData.target.result)

            console.log(newData.target.result.replace('"', ''))
            const response = CryptoJS.AES.decrypt(newData.target.result.replace('"', ''), 'sdaoi213@*#78&*&*Edsah&(821j3kbkdas*((')
