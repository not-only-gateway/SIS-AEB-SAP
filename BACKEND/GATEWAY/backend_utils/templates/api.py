...
class ApiView:
    ...
    def list_entries(self, db, instance, fields, sorts, offset, quantity):
        ...
        for index, i in enumerate(sorts):
            ...
            if i.get('desc', False):
                applied_sorts += getattr(instance, i.get('key', None)).name + ' DESC' + end
            else:
                applied_sorts += getattr(instance, i.get('key', None)).name + ' ASC' + end
            ...
            if relation is not None and relation.get('instance', None) is not None:
                try:
                    ...
                    if entity is not None:
                        ...
                        if type(entity) is dict and entity.get('_sa_instance_state', None) is not None:
                            del entity['_sa_instance_state']
                        data[relation.get('key', None)] = entity
    ...
    def list(self, request, db, base_query=[]):
        data = request.args
        if data is None:
            data = dict()
